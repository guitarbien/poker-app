...
- **翻牌前（僅評 RFI 情境）**：玩家在「輪到自己時前面**無人進池**（本手 preflop 尚無任何其他玩家 call 或 raise，僅有棄牌；盲注不算進池動作）」的開牌/棄牌決策，對照 `ranges.ts` → 標記「標準 / 過鬆（preflop-loose）/ 過緊（preflop-tight）」。**面對加注、面對 limper、盲注防守等情境第一版不評分**（開牌表基準是無人進池，硬套會產生錯誤教學；見 §12 與附錄 A）
