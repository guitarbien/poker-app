...
test('Equity 估算：越界停用作答鈕 → 輸入合法值作答 → 驗解說', async ({ page }) => {
  await page.goto('/');
  await page.evaluate(() => localStorage.clear());
  await page.reload();

  await page.getByRole('button', { name: '訓練' }).click();
  await page.getByTestId('start-equityGuess').click();

  const input = page.getByTestId('equity-input');
  await expect(input).toBeVisible({ timeout: 10_000 });

  // 越界（150）→ 作答鈕停用
  await input.fill('150');
  await expect(page.getByTestId('equity-submit')).toBeDisabled();

  // 合法值（50）→ 作答鈕可用 → 送出
  await input.fill('50');
  await expect(page.getByTestId('equity-submit')).toBeEnabled();
  await page.getByTestId('equity-submit').click();
